...
/** fetch wrapper that resends a known run id and captures the one the gateway mints. */
...
export function getLovableAiGatewayResponseHeaders(
  ...
  if (exposed.size > 0) headers.set("Access-Control-Expose-Headers", [...exposed].join(", "));
  return headers;
}
